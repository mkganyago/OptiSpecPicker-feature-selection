...
# set.seed(186)
# sfeats.rfe.lcc = selectFeatures(
#   learner = "regr.randomForest", task = optispect_best_task, resampling = rdesc.feat, control = ctrl.feat,
#   show.info = FALSE)
# 
# sfeats.rfe.lcc
# 
# analyzeFeatSelResult(sfeats.rfe.lcc)
# #----------------------------------------------------------
# Test the subset's predictive capability with RF
# Create a new task with selected variables
stfs_subset.rfe <- subset(S2_bands2, select = c(B11,NDVIre, RERI_705,B6))
head(stfs_subset.rfe) 

stfs_subset.rfe.data <- cbind(stfs_subset.rfe, y)
head(stfs_subset.rfe.data)

optispect_best_task <- makeRegrTask(id="rfe-rf-lcc", data = stfs_subset.rfe.data, target = "y")
optispect_best_task
#---------------------------------------------------------
...
set.seed(432)
model.split.rfe <- resample(learner = lrn_tune.model,task = optispect_best_task, resampling = rdesc, measures = list(mlr::mape, mlr::rmse, mlr::rsq))

print(model.split.rfe)

#------------------------------------------------------------------
#------------------------- Accuracy metrics -----------------------
## Accuracy metrics (from resampled data)
#Create a summary table from the aggregate values
SummaryTbl_test_metrics <- t(as.data.frame(model.split.rfe$aggr))
...
regr_result <-model.split.rfe$pred$data
#----------------------------------------------------------
...
# Select features
rdesc.feat = makeResampleDesc("CV", iters = 10)
sfeats.rfe.lai = selectFeatures(
  learner = "regr.randomForest", task = FCT_all_bothaville_data_task, resampling = rdesc.feat, control = ctrl.feat,
  show.info = FALSE)

sfeats.rfe.lai

analyzeFeatSelResult(sfeats.rfe.lai)
#-------------------------------------------------------------------------
# Test the subset's predictive capability with RF
# library
library(mlr)

stfs_subset.rfe <- subset(S2_bands, select = c(B3,B5,B6,B8A,B11))
head(stfs_subset.rfe) 

stfs_subset.rfe.data <- cbind(stfs_subset.rfe, LAI)
head(stfs_subset.rfe.data)

stfs_subset.rfe.data_task <- makeRegrTask(id="stfs-rf-lai",data = stfs_subset.rfe.data,target = "LAI")
stfs_subset.rfe.data_task
